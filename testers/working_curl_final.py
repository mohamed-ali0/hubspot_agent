...
def generate_working_curl():
    ...
    try:
        ...
        print(f"[OK] Server is running: {test_response.status_code}")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 201:
            print("[OK] Contact creation test PASSED!")
            result = response.json()
            print(f"   HubSpot ID: {result.get('hubspot_id')}")
        else:
            print(f"[ERROR] Contact creation test FAILED: {response.text}")
            return
    except Exception as e:
        print(f"[ERROR] Contact creation test ERROR: {e}")
        return
